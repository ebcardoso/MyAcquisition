...
public class Product : BaseModel
{
  ...
  public Product(int id, int brandId, string name, Units unit, string amount)
  {
    ModelValidationException.When(id < 0, "Field Id should to be greater than 0");
    Id = id;

    ValidateModel(brandId, name, unit, amount);
  }

  public Product(int brandId, string name, Units unit, string quantity)
  {
    ValidateModel(brandId, name, unit, quantity);
  }

  private void ValidateModel(int brandId, string name, Units unit, string quantity)
  {
    ModelValidationException.When(brandId < 0, "Field Id should to be greater than 0");
